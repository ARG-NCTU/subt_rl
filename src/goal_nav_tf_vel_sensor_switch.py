...
class GoalNav(object):
    def __init__(self):
        ...
        self.sub_mm_laser = rospy.Subscriber(
            "RL/denoised_mmwave",  LaserScan, self.cb_mm_laser, queue_size=1)
    ...
    def cb_laser(self, msg):
        ...
        if(self.count<=100):
            ...
            # print("240 degree")
            visual_laser = LaserScan()
            visual_laser = self.mmdata
            # visual_laser.ranges = ranges
            self.pub_visual_laser.publish(visual_laser)
            ranges = self.mmdata.ranges
            ranges = np.clip(ranges, 0, self.max_mm_dis)

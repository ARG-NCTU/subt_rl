...
class GoalNav(object):
    def __init__(self):
        ...
        self.mission = rospy.get_param("~mission", "forest")
        ...
        if self.mission == "cave_240":
            model_path = "/home/argsubt/subt-virtual/acme_logs/cave/snapshots/policy"
        elif self.mission == "curriculum_90":
            model_path = "/home/argsubt/subt-virtual/acme_logs/degree_90_curriculum/snapshots/policy"
        elif self.mission == "forest" :
            model_path = "/home/argsubt/subt-virtual/acme_logs/forest/snapshots/policy"
            print('model path:',model_path)
        elif self.mission == "garage" :
            model_path = "/home/argsubt/subt-virtual/acme_logs/garage_load/snapshots/policy"
            print('model path:',model_path)
    ...
    def cb_joy(self, msg):
        ...
        elif msg.buttons[y_button] == 1:
            self.count = 0
            self.get_record_json()


    def get_record_json(self):
        # get goal_list from json
        with open('/home/argsubt/subt-virtual/catkin_ws/src/subt-core/subt_rl/src/goal_list.json', 'r') as f:
            self.goal_list = json.load(f)
        print("get goal_list from json")
        print(self.goal_list) 
        self.goal = np.array(self.goal_list[0])
    ...
    def cb_laser(self, msg):
        ...
        ranges_scale = deepcopy(msg)
        
        # Scale for Jackal cause size is different from Husky
        ranges *= 0.75
        ranges = np.clip(ranges, 0, self.max_dis)
        
        ranges_scale.ranges = ranges.tolist()
        self.pub_visual_laser.publish(ranges_scale)
    ...
    def inference(self, event):
        ...
        if dis < self.goal_dis:
            ...
            try:
                self.goal = self.goal_list[self.count]
            except:
                print("goal_list is empty")    

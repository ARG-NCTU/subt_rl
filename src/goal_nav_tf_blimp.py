...
class GoalNav(object):
    def __init__(self):
        super().__init__()
        self.constant_height = PID_control("cool robot control", P=1, I=0.05, D=0.05)
        self.height = 0
    ...
    def inference(self, event):
        ...
        if dis < 1.5:
            rospy.loginfo("goal near")
            # self.goal = None
            cmd = Twist()
            self.constant_height.update(float(self.target_height-self.height))
            u = self.constant_height.output
            cmd.linear.z = u
            self.constant_height.update(float((self.goal-self.last_pos)[0]))
            u = self.constant_height.output
            cmd.linear.x = u
            self.constant_height.update(float((self.goal-self.last_pos)[1]))
            u = self.constant_height.output
            cmd.linear.y = u
            self.pub_cmd.publish(cmd)
            return
